...
export default function WishlistSection({ onClose }: WishlistSectionProps) {
    ...
    >
      ...
      <div className="flex-1">
        <div className="flex items-center space-x-2">
          <span className="text-gray-900 text-sm font-medium">{t('header.wishList')}</span>
        </div>
        <p className="text-gray-500 text-xs mt-0.5">
          {isLoggedIn ? "Your saved items" : "Login to view your wishlist"}
        </p>
      </div>
    </button>
  )
